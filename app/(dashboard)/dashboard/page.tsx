...
const statusTone: Record<ProductStatus, string> = {
  ...
  [ProductStatus.PUBLISHED]: "bg-emerald-100 text-emerald-700 ring-emerald-200/80",
  [ProductStatus.ARCHIVED]: "bg-neutral-100 text-neutral-600 ring-neutral-200/80",
};
  ...
  return (
    <div className="rounded-2xl bg-white p-5 shadow-sm ring-1 ring-neutral-200/60">
      <div className="flex items-center justify-between gap-4">
        ...
        <div className={`flex h-12 w-12 items-center justify-center rounded-xl ${accentClass ?? "bg-carbon text-white"}`}>
          {icon}
        </div>
      </div>
      {helper ? <p className="mt-4 text-xs text-neutral-500">{helper}</p> : null}
    </div>
  );
}
...
export default async function Page() {
  ...
  const publishedProducts = products.filter((product) => product.status === ProductStatus.PUBLISHED).length;
  const draftProducts = products.filter((product) => product.status === ProductStatus.DRAFT).length;
  const archivedProducts = products.filter((product) => product.status === ProductStatus.ARCHIVED).length;
  const featuredProducts = products.filter((product) => product.featured).length;
  const discountedProducts = products.filter((product) => (product.discountPercentage ?? 0) > 0);
  const lowStockCandidates = products.filter((product) => (product.stock ?? 0) <= LOW_STOCK_THRESHOLD);
  const lowStockHighlights = [...lowStockCandidates]
    .sort((a, b) => (a.stock ?? 0) - (b.stock ?? 0))
    .slice(0, 5);
  const totalUnitsAvailable = products.reduce((acc, product) => acc + (product.stock ?? 0), 0);
  ...
  const averageDiscount =
    discountedProducts.length > 0
      ? discountedProducts.reduce((acc, product) => acc + (product.discountPercentage ?? 0), 0) /
      ...
      <div className="mx-auto flex max-w-7xl flex-col gap-10 px-6 py-10">
        <section className="rounded-3xl bg-white px-6 py-8 shadow-sm ring-1 ring-neutral-200/60 sm:px-10">
          <div className="flex flex-col gap-6 lg:flex-row lg:items-end lg:justify-between">
            <div>
              <p className="text-xs font-semibold uppercase tracking-[0.3em] text-carbon/70">Panel general</p>
              <h1 className="mt-3 text-3xl font-semibold text-neutral-900 sm:text-4xl">Hola, {session.user.name} 👋</h1>
              <p className="mt-3 max-w-2xl text-sm text-neutral-500">
                Seguimiento rápido del catálogo, promociones y stock. Revisa los productos recientes o actúa sobre las alertas de inventario.
              </p>
            </div>
        ...
        <section className="grid gap-6 md:grid-cols-2 xl:grid-cols-4">
          ...
          <StatCard
            ...
            helper={averageRating ? `Promedio ${averageRating.toFixed(1)} / 5` : "Aún sin reseñas registradas"}
          ...
          <div className="space-y-6 lg:col-span-2">
            <div className="overflow-hidden rounded-2xl bg-white shadow-sm ring-1 ring-neutral-200/60">
              <div className="flex items-center justify-between px-6 py-4">
                <div>
                  <h2 className="text-sm font-semibold text-neutral-900">Últimos productos cargados</h2>
                  <p className="text-xs text-neutral-500">Ordenados del más reciente al más antiguo</p>
                </div>
              </div>
                    ...
                    {recentProducts.map((product) => {
                      ...
                      return (
                        <tr key={product.id} className="hover:bg-neutral-50/80">
                          ...
                          <td className="px-6 py-4">
                            <span className={`inline-flex items-center gap-2 rounded-full px-3 py-1 text-xs font-semibold ring-1 ${statusTone[product.status]}`}>
                          ...
                          <td className="px-6 py-4">
                            <div className="text-xs text-neutral-500">
                              {product.categories.length > 0
                                ? product.categories.map((catLink) => catLink.category.name).join(", ")
                                : "Sin categoría"}
                            </div>
                          </td>
                          <td className="px-6 py-4 text-right">
                            <span className="font-medium text-neutral-900">{product.stock ?? 0}</span>
                          </td>
                          <td className="px-6 py-4 text-right">
                            <div className="font-semibold text-neutral-900">{formatCurrency(Number(product.price ?? 0))}</div>
                            {product.discountPercentage ? (
                              <span className="text-xs text-emerald-600">{product.discountPercentage}% OFF</span>
                            ) : null}
                          </td>
                        </tr>
                      );
            ...
            <div className="grid gap-6 md:grid-cols-2">
              <div className="rounded-2xl bg-white p-6 shadow-sm ring-1 ring-neutral-200/60">
                <h2 className="text-sm font-semibold text-neutral-900">Estado del catálogo</h2>
                <p className="mt-1 text-xs text-neutral-500">Distribución de los productos por estado</p>

                <ul className="mt-6 space-y-4">
                  {statusDistribution.map((item) => {
                    const percentage = totalProducts > 0 ? Math.round((item.value / totalProducts) * 100) : 0;
                    return (
                      <li key={item.key}>
              ...
              <div className="rounded-2xl bg-white p-6 shadow-sm ring-1 ring-neutral-200/60">
                <h2 className="text-sm font-semibold text-neutral-900">Promociones activas</h2>
                <p className="mt-1 text-xs text-neutral-500">Resumen de descuentos aplicados al catálogo</p>
                ...
                <div className="mt-6 space-y-4 text-sm">
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <span className="font-medium text-neutral-900">
                      {discountedProducts.length > 0 ? `${averageDiscount.toFixed(1)}%` : "—"}
                    </span>
                  </div>
                </div>
              </div>
          ...
          <div className="space-y-6">
            <div className="rounded-2xl bg-white p-6 shadow-sm ring-1 ring-neutral-200/60">
              <div className="flex items-center justify-between gap-2">
                <h2 className="text-sm font-semibold text-neutral-900">Alertas de stock</h2>
                <span className="text-xs text-neutral-500">≤ {LOW_STOCK_THRESHOLD} unidades</span>
              </div>
              <ul className="mt-5 space-y-4 text-sm">
                {lowStockHighlights.length === 0 ? (
                  <li className="text-neutral-500">No hay alertas. El inventario está saludable.</li>
                ) : (
            ...
            <div className="rounded-2xl bg-white p-6 shadow-sm ring-1 ring-neutral-200/60">
              <h2 className="text-sm font-semibold text-neutral-900">Próximos pasos sugeridos</h2>
              <ul className="mt-5 space-y-3 text-sm text-neutral-600">
                <li className="flex items-start gap-3">
                  <ClipboardList className="mt-0.5 h-4 w-4 text-carbon" />
                  <span>Publica los {draftProducts} productos que siguen en borrador para ampliar el catálogo.</span>
                </li>
                <li className="flex items-start gap-3">
                  <AlertTriangle className="mt-0.5 h-4 w-4 text-amber-500" />
                  <span>Revisa reposición para {lowStockCandidates.length} productos con stock bajo.</span>
                </li>
                <li className="flex items-start gap-3">
                  <Sparkles className="mt-0.5 h-4 w-4 text-emerald-500" />
                  <span>Define campañas para los {discountedProducts.length} productos con descuento activo.</span>
                </li>
              </ul>
            </div>
          </div>
